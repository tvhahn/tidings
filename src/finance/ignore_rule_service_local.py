"""SQLite implementation of IgnoreRuleService — mirrors the DynamoDB public API."""
...
from src.finance.ignore_rule_service import IgnoreRuleServiceBase
...
class IgnoreRuleServiceLocal(IgnoreRuleServiceBase):
    """CRUD for merchant auto-ignore rules in SQLite."""
    ...
    def get_rules(self) -> dict[str, Any] | None:
        ...
        try:
            row = conn.execute(
                ...
                (self.USER_PK, self.IGNORE_RULES_SK),
            ).fetchone()
            if not row:
                return None
    ...
    def _put_all_with_dismissed(
        ...
        try:
            ...
            if expected_version is None:
                ...
                cursor = conn.execute(
                    CONFIG_UPDATE_EXTRA_SQL,
                    (data_str, new_version, now, extra_str, self.USER_PK, self.IGNORE_RULES_SK, expected_version),
                )
