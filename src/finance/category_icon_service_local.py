"""SQLite implementation of CategoryIconService — mirrors the DynamoDB public API."""
...
from src.finance.category_icon_service import CategoryIconServiceBase
...
class CategoryIconServiceLocal(CategoryIconServiceBase):
    """CRUD for category icon overrides in SQLite."""
    ...
    def _put_all(self, data: dict[str, str], expected_version: int | None) -> int:
        ...
        try:
            ...
            if expected_version is None:
                conn.execute(
                    CONFIG_INSERT_SQL,
                    (self.USER_PK, self.ICONS_SK, data_str, new_version, now),
                )
            else:
                cursor = conn.execute(
                    CONFIG_UPDATE_SQL,
                    (data_str, new_version, now, self.USER_PK, self.ICONS_SK, expected_version),
                )
