...
The review screen suggests a category from your [rules and aliases](/using/categorization/), so a merchant you have already filed shows up filed rather than as miscellaneous. A row still uncategorized when you import runs through the same tiers an emailed transaction does, AI included if you have it enabled. Merchants covered by an auto-ignore rule arrive ignored.
